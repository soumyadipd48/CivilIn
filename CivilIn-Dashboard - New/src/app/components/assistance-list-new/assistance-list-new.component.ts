...
@Component({
  selector: 'app-assistance-list-new',
  templateUrl: './assistance-list-new.component.html',
  styleUrls: ['./assistance-list-new.component.css']
})
export class AssistanceListNewComponent implements OnInit {
  classToggled = false;
  ...
  wish : string = '';
  orderPriority? = '';
  Assistance_count_all = 0;
  Assistance_count_daily = 0;
  Assistance_count_pending_daily? : Assistance[];
  Assistance_count_pending = 0;
  Assistance_count_pending_array? : Assistance[];
  Assistance_count_progress = 0;
  Assistance_count_progress_array? : Assistance[];
  Assistance_count_completed = 0;
  Assistance_count_completed_array? : Assistance[];
  // ALL GRP names
  allGRP = [
    {
      id: 'HWH-GRP',
  ...
  constructor(private assistanceService : AssistanceService, private titleService : Title, private modalService : BsModalService, private userService : UserService, private datePipe : DatePipe) {
  ...
  public retrieveAssistance() : void {
    this.assistanceService.getAllAssintance().snapshotChanges().pipe(
      ...
      // this.Assistance_count_pending_daily = this.Assistance.filter(
      //   item => item.status === 'Pending'
      // )
      this.Assistance_count_pending_array = this.Assistance.filter(
        item => item.status === 'Pending'
      )
      this.Assistance_count_progress_array = this.Assistance.filter(
        item => item.status === 'In Progress'
      )
      this.Assistance_count_completed_array = this.Assistance.filter(
        item => item.status === 'Completed'
      )

      this.Assistance_count_all = this.Assistance.length;
      this.Assistance_count_pending = this.Assistance_count_pending_array.length;
      this.Assistance_count_progress = this.Assistance_count_progress_array.length;
      this.Assistance_count_completed = this.Assistance_count_completed_array.length;

      console.log("Assistance count"+this.Assistance.length);
      ...
      this.Assistance.forEach( (element) => {
        ...
        if(month==todayMonth && day==todayDate && year==todayYear){
          this.Assistance_count_daily = this.Assistance_count_daily+1;
        }
        console.log(this.Assistance_count_daily + " daily count");
      });
  ...
  public openModal_progress(template : TemplateRef<any>, assistance : Assistance)
  {
    ...
    this.modalAssistance_AssistanceDesc = assistance.assRemarks;
    this.modalAssistance_remarks_after_completion = assistance.remarks;
